...
class FamilyJodi : AppCompatActivity() {
            ...
            if ((sessionType.equals("open")) || (closeRadioButton.isChecked && sessionType.equals("close"))) {
                list = ArrayList()
                    ...
                    for (value in valueList) {
                        list.add(
                            BetItem(
                                pointsEditText.text.toString().toInt(),
            ...
            object : ApiResponse {
                override fun onSuccess(jsonObject: JsonObject) {
                    if(!jsonObject.isJsonNull){
                        ...
                        {setwallet()
                            Toast.makeText(this@FamilyJodi,"Bet submitted", Toast.LENGTH_SHORT).show()
                            pointsEditText.setText("")}
                        else{
                            Toast.makeText(this@FamilyJodi,jsonObject.get("message").toString(),
                                Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(this@FamilyJodi,"Failed", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(failure: String) {
                    Toast.makeText(this@FamilyJodi,failure, Toast.LENGTH_SHORT).show()
                }
    ...
    private fun setupspinner() {
        ...
        valueList=ArrayList()
        val keyList=GameData.family_jodiList().sorted()
        ...
        digitsSpinner.onItemSelectedListener=object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                valueList= GameData.family_jodi(selectedItem).toMutableList()
                valueList.sortBy { it }
                var patty:String=""
                for (item in valueList){
                    patty += "${String.format("%02d", item)}, "
                }
