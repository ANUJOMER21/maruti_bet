...
class DpGame : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_dp_game)
        initview()
...
openRadioButton.isChecked=true
            ...
            object : ApiResponse {
                override fun onSuccess(jsonObject: JsonObject) {
                    if(!jsonObject.isJsonNull){
                        ...
                        {setwallet()
                            Toast.makeText(this@DpGame,"Bet submitted", Toast.LENGTH_SHORT).show()
                            pointsEditText.setText("")}
                        else{
                            Toast.makeText(this@DpGame,jsonObject.get("message").toString(),
                                Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(this@DpGame,"Failed", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(failure: String) {
                    Toast.makeText(this@DpGame,failure, Toast.LENGTH_SHORT).show()
                }
    ...
    private fun setupspinner() {
        ...
        valueList=ArrayList()
        val keyList=GameData.dpkeys().sorted()
        ...
        digitsSpinner.onItemSelectedListener=object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                valueList= (GameData.Dp(selectedItem) as MutableList<Int>?)!!
                valueList.sortBy { it }
                var patty:String=""
