...
class redfamilyJodi : AppCompatActivity() {
    ...
    lateinit var valueList:MutableList<String>
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_redfamily_jodi)
        initview()
            ...
            object : ApiResponse {
                override fun onSuccess(jsonObject: JsonObject) {
                    if(!jsonObject.isJsonNull){
                        ...
                        {
                            Toast.makeText(this@redfamilyJodi,"Bet submitted", Toast.LENGTH_SHORT).show()
                            pointsEditText.setText("")
                        setwallet()}
                        else{
                            Toast.makeText(this@redfamilyJodi,jsonObject.get("message").toString(),
                                Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(this@redfamilyJodi,"Failed", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(failure: String) {
                    Toast.makeText(this@redfamilyJodi,failure, Toast.LENGTH_SHORT).show()
                }
    ...
    private fun setupspinner() {
        ...
        valueList=ArrayList()
        val keyList=GameData.redfamilykey().sorted()
        ...
        digitsSpinner.onItemSelectedListener=object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                valueList= GameData.redfamilyList(selectedItem)
                valueList.sortBy { it }
                var patty:String=""
