...
class Cyclepatti : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_cyclepatti)
        initview()
...
backBtn.setOnClickListener { finish() }
            ...
            object : ApiResponse {
                override fun onSuccess(jsonObject: JsonObject) {
                    if(!jsonObject.isJsonNull){
                        ...
                        {setwallet()
                            Toast.makeText(this@Cyclepatti,"Bet submitted", Toast.LENGTH_SHORT).show()
                            pointsEditText.setText("")}
                        else{
                            Toast.makeText(this@Cyclepatti,jsonObject.get("message").toString(),
                                Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(this@Cyclepatti,"Failed", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(failure: String) {
                    Toast.makeText(this@Cyclepatti,failure, Toast.LENGTH_SHORT).show()
                }
    ...
    private fun setupspinner() {
        ...
        valueList=ArrayList()
        val keyList=GameData.cyclepattiitem().sorted()
        ...
        digitsSpinner.onItemSelectedListener=object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                valueList= GameData.cyclepattiList(selectedItem).toMutableList()
                valueList.sortBy { it }
                var patty:String=""
